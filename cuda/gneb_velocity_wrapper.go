...
// CUDA handle for velocity kernel
var velocity_code cu.Function

// Stores the arguments for velocity kernel invocation
type velocity_args_t struct {
...
// Stores the arguments for velocity kernel invocation
var velocity_args velocity_args_t

func init() {
	// CUDA driver kernel call wants pointers to arguments, set them up once.
	velocity_args.argptr[0] = unsafe.Pointer(&velocity_args.arg_vx)
	velocity_args.argptr[1] = unsafe.Pointer(&velocity_args.arg_vy)
	velocity_args.argptr[2] = unsafe.Pointer(&velocity_args.arg_vz)
	velocity_args.argptr[3] = unsafe.Pointer(&velocity_args.arg_kx)
	velocity_args.argptr[4] = unsafe.Pointer(&velocity_args.arg_ky)
	velocity_args.argptr[5] = unsafe.Pointer(&velocity_args.arg_kz)
	velocity_args.argptr[6] = unsafe.Pointer(&velocity_args.arg_mx)
	velocity_args.argptr[7] = unsafe.Pointer(&velocity_args.arg_my)
	velocity_args.argptr[8] = unsafe.Pointer(&velocity_args.arg_mz)
	velocity_args.argptr[9] = unsafe.Pointer(&velocity_args.arg_m0x)
	velocity_args.argptr[10] = unsafe.Pointer(&velocity_args.arg_m0y)
	velocity_args.argptr[11] = unsafe.Pointer(&velocity_args.arg_m0z)
	velocity_args.argptr[12] = unsafe.Pointer(&velocity_args.arg_N)
	velocity_args.argptr[13] = unsafe.Pointer(&velocity_args.arg_Nz)
}

// Wrapper for velocity CUDA kernel, asynchronous.
func k_velocity_async(vx unsafe.Pointer, vy unsafe.Pointer, vz unsafe.Pointer, kx unsafe.Pointer, ky unsafe.Pointer, kz unsafe.Pointer, mx unsafe.Pointer, my unsafe.Pointer, mz unsafe.Pointer, m0x unsafe.Pointer, m0y unsafe.Pointer, m0z unsafe.Pointer, N int, Nz int, cfg *config) {
	...
	velocity_args.Lock()
	defer velocity_args.Unlock()

	if velocity_code == 0 {
		velocity_code = fatbinLoad(velocity_map, "velocity")
	}

	velocity_args.arg_vx = vx
	velocity_args.arg_vy = vy
	velocity_args.arg_vz = vz
	velocity_args.arg_kx = kx
	velocity_args.arg_ky = ky
	velocity_args.arg_kz = kz
	velocity_args.arg_mx = mx
	velocity_args.arg_my = my
	velocity_args.arg_mz = mz
	velocity_args.arg_m0x = m0x
	velocity_args.arg_m0y = m0y
	velocity_args.arg_m0z = m0z
	velocity_args.arg_N = N
	velocity_args.arg_Nz = Nz

	args := velocity_args.argptr[:]
	cu.LaunchKernel(velocity_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream0, args)
...
// maps compute capability on PTX code for velocity kernel.
var velocity_map = map[int]string{0: "",
	50: velocity_ptx_50}

// velocity PTX code for various compute capabilities.
const (
	velocity_ptx_50 = `
.version 7.5
.target sm_50
